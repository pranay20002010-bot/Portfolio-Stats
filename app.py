...
def growth_chart(
    ...
    ax.plot(cumret.index, cumret.values, color="#290084", linewidth=1.8, label=name)
    ...
    ax.set_title(f"Performance vs. Benchmarks — {name}", fontsize=9, color="#290084", fontweight="bold")
...
APP_BUILD_TAG = "build-2026-08-19-branding-report-overhaul-13"
    ...
    if not holdings.empty and held_tickers:
        ...
        if sector_split.empty:
            ...
            sector_split.plot.pie(
                ...
                colors=wedge_colors[: len(sector_split)], textprops={"fontsize": 9},
