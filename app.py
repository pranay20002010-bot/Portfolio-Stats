...
APP_BUILD_TAG = "build-2026-08-19-oversold-backfill-5"
...
with overview_tab:
    ...
    if not consistency_issues.empty:
        ...
        with st.expander("Why do these disagree? (raw dates behind the mismatch)"):
            st.caption(
                "For each affected portfolio: its own transaction date range, the downloaded price "
                "calendar's date range, and whether/how far apart they sit. If the transaction dates sit "
                "entirely outside the price calendar's range, that's the root cause — likely because every "
                "ticker that portfolio holds failed to download (see the missing-tickers list above), so "
                "there's no price calendar covering its actual holding period at all."
            )
            debug_rows = []
            for _p in sorted(consistency_issues["portfolio"].unique()):
                _txns = df[df["portfolio"].astype(str).str.strip() == str(_p).strip()]
                _tickers_held = consistency_issues.loc[consistency_issues["portfolio"] == _p, "ticker_yf"].tolist()
                _tickers_priced = [t for t in _tickers_held if t in prices.columns]
                debug_rows.append(
                    {
                        "portfolio": _p,
                        "txn_date_min": _txns["date"].min().date().isoformat() if not _txns.empty else "N/A",
                        "txn_date_max": _txns["date"].max().date().isoformat() if not _txns.empty else "N/A",
                        "price_calendar_min": prices.index.min().date().isoformat() if not prices.empty else "N/A",
                        "price_calendar_max": prices.index.max().date().isoformat() if not prices.empty else "N/A",
                        "mismatched_tickers": len(_tickers_held),
                        "of_which_have_price_data": len(_tickers_priced),
                    }
                )
            st.dataframe(pd.DataFrame(debug_rows), use_container_width=True, hide_index=True)

        with st.expander("Raw transactions + computed daily position for a sample ticker"):
            st.caption(
                "Every raw transaction row behind one mismatched (portfolio, ticker) pair, plus the last "
                "10 dates of what build_daily_positions actually computed for it day by day. This shows "
                "exactly where the number goes wrong, rather than guessing at the mechanism."
            )
            _sample = consistency_issues.iloc[0]
            _sp, _st_ticker = _sample["portfolio"], _sample["ticker_yf"]
            st.write(f"Sample: portfolio = **{_sp}**, ticker = **{_st_ticker}**")
            _sample_txns = df[
                (df["portfolio"].astype(str).str.strip() == str(_sp).strip()) & (df["ticker_yf"] == _st_ticker)
            ][["date", "action", "quantity", "price", "is_opening"]].sort_values("date")
            st.write("Raw transaction rows:")
            st.dataframe(_sample_txns, use_container_width=True, hide_index=True)
            _sample_holdings = daily_positions.get(str(_sp), pd.DataFrame())
            if _st_ticker in _sample_holdings.columns:
                st.write("Computed daily position (last 10 dates):")
                _tail = _sample_holdings[_st_ticker].tail(10).to_frame("computed_qty")
                st.dataframe(_tail, use_container_width=True)
            else:
                st.warning(f"'{_st_ticker}' isn't even a column in this portfolio's computed daily_positions.")
