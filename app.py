    ...
    if tickers_in_portfolio:
        ...
        if stock_rets:
            ...
            with w1:
                ...
                st.dataframe(winners.to_frame("Return").applymap(_fmt_pct), use_container_width=True)
            with w2:
                st.caption("Top 5 laggards")
                st.dataframe(laggards.to_frame("Return").applymap(_fmt_pct), use_container_width=True)
        else:
