    ...
    *,
    ...
    if sector_map is not None:
        ...
        if not exp.empty:
            ...
            wedge_colors = plt.cm.tab20.colors
            exp.plot.pie(
                ax=ax, autopct=_make_autopct_with_counts(counts), startangle=90,
                colors=wedge_colors[: len(exp)], textprops={"fontsize": 9},
...
APP_BUILD_TAG = "build-2026-08-19-static-sectors-capcaps-15"
...
with client_tab:
    ...
    # Discover all unique tickers currently present in the uploaded workbook.
    ...
    st.caption(
        "Enter the sector for each unique ticker below. "
        "These classifications are used for the sector exposure chart."
    )
    ...
    if all_unique_tickers:
        ...
        for ticker in all_unique_tickers:
            ...
            for key, value in SECTOR_MAP.items():
                if str(key).strip().upper().replace(".NS", "").replace(".BO", "") == ticker:
                    existing = str(value)
                    break
            ...
            sector = st.text_input(
                ticker,
                value=existing,
                key=f"sector_classification_{ticker}",
                label_visibility="collapsed",
                placeholder=f"{ticker} sector",
            )
            manual_sector_map[ticker] = sector.strip() if sector.strip() else "Unknown"

        effective_sector_map = manual_sector_map

        if not holdings.empty and held_tickers:
            sector_split, sector_counts = sector_exposure_weights_and_counts(
                holdings, prices, effective_sector_map
            )

            if sector_split.empty:
                st.info("Sector data unavailable for this portfolio's holdings.")
            else:
                # Keep the pie chart, but move ALL labels into a clean legend.
                # Only percentages are shown on the slices, preventing the
                # previous collision between sector names and percentages.
                fig_sec, ax_sec = plt.subplots(
                    figsize=(9, 6.5)
                )

                wedges, _, autotexts = ax_sec.pie(
                    sector_split.values,
                    startangle=90,
                ...
                # Legend contains the full sector name, percentage and number
                # of holdings. This is much easier to scan than labels around
                # the circumference of the pie.
                legend_labels = [
                    f"{sector}  •  {weight:.1f}%  •  {int(sector_counts.get(sector, 0))} holding"
                    + ("" if int(sector_counts.get(sector, 0)) == 1 else "s")
                    for sector, weight in zip(
                        sector_split.index,
                        sector_split.values * 100,
                    )
                ]
        ...
        st.info("No tickers found in the uploaded workbook.")
