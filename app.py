...
# Portfolio-level metrics table (annualized + risk metrics) — uses TWR
metrics_rows = []
...
with client_tab:
    ...
    m2.metric("Beta (Nifty 500)", _fmt_float(client_metrics.get("Beta (Nifty 500)")))
    m3.metric("Jensen Alpha", _fmt_pct(client_metrics.get("Jensen Alpha")))
    ...
    if holdings.empty:
        ...
        # Merge in avg buy price from current_positions (FIFO)
        cur_pos = positions[positions["portfolio"] == portfolio][["ticker_yf", "qty", "avg_buy_price"]].copy()
        cur_pos = cur_pos.set_index("ticker_yf").rename(columns={"qty": "_qty", "avg_buy_price": "Avg Buy Price (₹)"})
        latest = latest.join(cur_pos[["Avg Buy Price (₹)"]], how="left")
    ...
    currently_held = set(
        positions.loc[positions["portfolio"] == portfolio, "ticker_yf"].tolist()
    )
