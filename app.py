...
with client_tab:
    ...
    if not holdings.empty:
        ...
        if cap_split.empty:
            ...
            PIE_COLOR_MAP = {"Large Cap": "#1f77b4", "Mid Cap": "#ff7f0e", "Small Cap": "#2ca02c", "Unknown": "#B0B0B0"}
            pie_df = cap_split.mul(100).round(1).to_frame("Weight").reset_index().rename(columns={"index": "Bucket"})
            fig_cap = px.pie(
                pie_df, names="Bucket", values="Weight", color="Bucket", color_discrete_map=PIE_COLOR_MAP
            )
            fig_cap.update_traces(textposition="inside", textinfo="percent+label", sort=False)
            fig_cap.update_layout(height=380, margin=dict(l=20, r=20, t=20, b=20), showlegend=False)
