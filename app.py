...
def build_daily_ledger(
    ...
    Cash accounting:
      ...
      - "Initial Holdings" rows (tagged `is_opening=True` in the combined
        transactions frame — see prepare_transactions) never hit cash via
        the normal BUY debit. Instead, if the sheet has a MarketValue
        column, Day-0 cash is bootstrapped as:

            opening_cash = sum(MarketValue across that portfolio's Initial
                               Holdings rows) - (those quantities valued at
                               StartDate prices)

        i.e. "whatever part of the recorded opening value isn't accounted
        for by the priced equity is cash" — the same bootstrap the
        reference PortfolioTWR.py uses. If MarketValue isn't present, this
        falls back to assuming zero starting cash (the sheet is treated as
        fully invested with no recorded cash cushion).
    ...
    for portfolio in sorted(df["portfolio"].unique().tolist()):
        ...
        # Bootstrap Day-0 cash from Initial Holdings' MarketValue, if present.
        cash = 0.0
        if has_is_opening and has_opening_mv:
            opening_rows = txns[txns["is_opening"] == True]  # noqa: E712
            opening_mv_sum = pd.to_numeric(opening_rows["opening_market_value"], errors="coerce").sum()
            if pd.notna(opening_mv_sum) and opening_mv_sum > 0:
                ...
                for tkr, qty in opening_qty.items():
                    if tkr in start_prices.index and pd.notna(start_prices[tkr]):
                        opening_equity += float(qty) * float(start_prices[tkr])
                cash = float(opening_mv_sum) - opening_equity
...
def compute_ledger_twr(ledger: pd.DataFrame) -> pd.Series:
    ...
    A day's return is left NaN (not zero, not computed) when either the
    prior or current day's cash balance is negative. Negative cash means a
    BUY wasn't matched by a recorded deposit (see build_daily_ledger) — on
    such a day, "Portfolio Value" doesn't represent real economic value, it
    represents equity value minus an accounting shortfall, and computing a
    return off that non-representative base produces a distorted swing that
    isn't a real gain or loss. Silently including it is what turns one data
    gap into wildly wrong Sharpe/Sortino/annualized-return numbers for the
    whole portfolio — better to skip the day and let the return series have
    a gap than let one bad day poison every downstream statistic.

    Also NaN when the prior day's portfolio value was at or below zero, for
    the same reason (no meaningful return from a non-positive base). Valid
    returns are capped at ±25%/day, matching the price-only method's limit.
    """
...
def download_prices(
    ...
    paths fail across every retry, for both the BSE and .NS candidate.

    Unlike the previous version, the actual exception (or "empty response")
    from the LAST attempt is kept per failed ticker in `failure_reasons` —
    a bare `except: pass` here means a systemic problem (a broken yfinance
    version, a blocked/rate-limited session, an auth/cookie requirement)
    looks identical to a genuinely bad ticker, which is exactly what makes
    "40 tickers missing" including names like RELIANCE/HDFCBANK/SBIN
    impossible to diagnose without seeing the real error text.
    """
    import time
    ...
    def _download_with_retries(sym: str) -> tuple[Optional[pd.Series], Optional[str]]:
        ...
        for attempt in range(max_retries):
            ...
            try:
                hist = yf_module.Ticker(sym).history(start=start, auto_adjust=True)
