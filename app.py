...
def _make_autopct_with_counts(counts: pd.Series):
    ...
    stocks)'. matplotlib invokes autopct once per wedge in the same order
...
def cap_split_pie_image(cap_split: pd.Series) -> Optional[Image]:
    ...
    color_map = {"Large Cap": "#290084", "Mid Cap": "#F6BF02", "Small Cap": "#1D9951", "Unknown": "#B0B0B0"}
...
APP_BUILD_TAG = "build-2026-08-19-pptx-period-metrics-triangle-corr-14"
...
with client_tab:
    ...
    if not holdings.empty:
        ...
        if cap_split.empty:
            ...
            cap_color_map = {"Large Cap": "#290084", "Mid Cap": "#F6BF02", "Small Cap": "#1D9951", "Unknown": "#B0B0B0"}
    ...
    if not holdings.empty and held_tickers:
        ...
        if sector_split.empty:
            ...
            sector_split.plot.pie(
                ax=ax_sec, autopct=_make_autopct_with_counts(sector_counts), startangle=90,
                colors=wedge_colors[: len(sector_split)], textprops={"fontsize": 8.5},
                pctdistance=0.75, labeldistance=1.08,
            )
            ax_sec.set_ylabel("")
