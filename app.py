...
def top_holdings_table(
    ...
    """
    Every currently-held ticker (no top-N cutoff), showing each stock's own
    company Market Cap (not the position's market value) alongside its
    Weight — % of TRUE total portfolio value (equity + cash), not just the
    sum of equity positions shown here. Dividing by equity alone silently
    inflates every weight by excluding cash from the denominator.

    An ETF or REIT doesn't have a company-equity market cap the way a
    stock does (ETF: none at all: REIT: Yahoo often just doesn't have it)
    — those rows show a blank Market Cap rather than a guessed or
    zero value, via ASSET_CLASS_MAP.
    ...
    for ticker, val in mv.items():
        ...
        if asset_class in ("ETF", "REIT"):
            mcap_str = ""
        elif market_caps is not None and ticker in market_caps.index and pd.notna(market_caps.get(ticker)):
            mcap_str = fmt_inr(float(market_caps[ticker]))
        else:
            mcap_str = ""
...
APP_BUILD_TAG = "build-2026-08-19-holdings-sector-cashflows-layout-19"
        ...
        if held_tickers:
            ...
            for ticker in mv.sort_values(ascending=False).index:
                ...
                if asset_class in ("ETF", "REIT"):
                    ...
                    mcap_val = float(mkt_caps.get(ticker)) if ticker in mkt_caps.index and pd.notna(mkt_caps.get(ticker)) else np.nan
                weight_val = mv[ticker] / denom * 100 if denom and abs(denom) > 1e-9 else np.nan
                holdings_rows.append({"Ticker": ticker.replace(".NS", "").replace(".BO", ""), "Market Cap": mcap_val, "Weight": weight_val})
