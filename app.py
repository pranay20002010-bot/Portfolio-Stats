...
def top_holdings_table(
    ...
    """
    Every currently-held ticker (no top-N cutoff), showing each stock's
    market-cap CLASSIFICATION (Large/Mid/Small Cap) — not the raw rupee
    market-cap figure — alongside its Weight — % of TRUE total portfolio
    value (equity + cash), not just the sum of equity positions shown
    here. Dividing by equity alone silently inflates every weight by
    excluding cash from the denominator.

    An ETF or REIT doesn't have a company-equity market cap the way a
    stock does, so the classification doesn't apply — those rows show a
    blank Market Cap rather than the literal "ETF"/"REIT" label, via
    ASSET_CLASS_MAP (through categorize_market_caps_inr).

    Ends with a "Cash" row (blank Market Cap, its own Weight) so the
    table's weights are a complete picture of the portfolio, not just its
    equity sleeve — matching sector_exposure_weights_and_counts' Cash
    bucket, which uses the same total-value denominator.
    """
    tickers = [t for t in holdings.columns if t in prices.columns]
    if not tickers:
        return None
    ...
    latest_h = holdings.iloc[-1][tickers]
    latest_p = prices.loc[prices.index[-1], tickers]
    mv_all = latest_h * latest_p
    mv = mv_all[mv_all > 0].sort_values(ascending=False)

    if portfolio_value is not None and pd.notna(portfolio_value) and abs(portfolio_value) > 1e-9:
        denom = float(portfolio_value)
    else:
        denom = mv_all.sum()  # fallback: % of shown equity only, no cash

    categories = categorize_market_caps_inr(market_caps) if market_caps is not None else pd.Series(dtype=object)

    rows = [["Ticker", "Market Cap", "Weight"]]
    for ticker, val in mv.items():
        weight_str = f"{val/denom*100:.1f}%" if denom and pd.notna(denom) and abs(denom) > 1e-9 else "N/A"
        cat = categories.get(ticker) if not categories.empty else None
        mcap_str = "" if cat is None or cat in ("ETF", "REIT") else cat
        rows.append([ticker.replace(".NS", "").replace(".BO", ""), mcap_str, weight_str])

    if cash is not None and pd.notna(cash):
        cash_weight = f"{cash/denom*100:.1f}%" if denom and abs(denom) > 1e-9 else "N/A"
        rows.append(["Cash", "", cash_weight])
    ...
    if cash is not None and pd.notna(cash):
        last = len(rows) - 1
        row_styles.append(("FONTNAME", (0, last), (-1, last), TITLE_STYLE.fontName))
        row_styles.append(("LINEABOVE", (0, last), (-1, last), 0.8, DARK_TEAL))

    t = Table(rows, colWidths=[70 * mm, 65 * mm, 30 * mm], repeatRows=1)
...
def sector_exposure_weights_and_counts(
    ...
    If `cash` and `portfolio_value` are both given, a "Cash" bucket is
    added sized to the actual cash balance, and every weight is expressed
    as a % of TRUE total portfolio value (equity + cash) rather than %
    of equity alone — this is what makes this chart's percentages match
    the Holdings table's Weight column, which uses the same total-value
    denominator. Without both of those, falls back to % of equity only.
    """
    tickers = [t for t in holdings.columns if t in prices.columns]
    ...
    have_cash = cash is not None and pd.notna(cash) and cash > 1e-9
    if mv.empty and not have_cash:
        return pd.Series(dtype=float), pd.Series(dtype=int)

    if not mv.empty:
        sector_of = pd.Series(mv.index, index=mv.index).map(sector_map).fillna("Unknown")
        exp = mv.groupby(sector_of).sum()
        counts = sector_of.value_counts().reindex(exp.index).fillna(0).astype(int)
    else:
        exp = pd.Series(dtype=float)
        counts = pd.Series(dtype=int)

    use_total_denom = portfolio_value is not None and pd.notna(portfolio_value) and abs(portfolio_value) > 1e-9
    denom = float(portfolio_value) if use_total_denom else exp.sum()

    if have_cash:
        exp = pd.concat([exp, pd.Series({"Cash": float(cash)})])
        # NaN (not 0) count for Cash — it isn't "0 stocks", it's not a
        # stock-count concept at all; _make_autopct_with_counts skips the
        # "(N)" suffix entirely for a NaN count instead of showing "(0)".
        counts = pd.concat([counts, pd.Series({"Cash": np.nan})])

    exp = exp.sort_values(ascending=False)
    counts = counts.reindex(exp.index)
    weights = exp / denom if denom else exp
...
def _make_autopct_with_counts(counts: pd.Series):
    ...
    A NaN count (used for a "Cash" bucket, which isn't a stock count at
    all) omits the parenthetical entirely rather than showing '(0)' or
    '(nan)'. matplotlib invokes autopct once per wedge in the same order
    as the input data, so a simple counter closure over `counts` (already
    ...
    def _autopct(pct):
        idx = state["i"]
        n = values[idx] if idx < len(values) else None
        state["i"] += 1
        if n is None or (isinstance(n, float) and pd.isna(n)):
            return f"{pct:.1f}%"
        return f"{pct:.1f}% ({int(n)})"
...
def sector_pie_figure_clean(exp: pd.Series):
    ...
    colors_used = []
    palette_i = 0
    for sector in exp.index:
        if sector == "Cash":
            colors_used.append("#B0B0B0")  # fixed neutral gray, always recognizable at a glance
        else:
            colors_used.append(wedge_colors[palette_i % len(wedge_colors)])
            palette_i += 1
...
def sector_pie_image(
    ...
    exp, counts = sector_exposure_weights_and_counts(holdings, prices, sector_map, cash=cash, portfolio_value=portfolio_value)
    if exp.empty:
        return None

    fig, colors_hex = sector_pie_figure_clean(exp)
    pie_img = make_chart_image(fig, width_mm=80, height_mm=80)

    rows = [["", "Sector", "%", "Stocks"]]
    for sector, weight in exp.items():
        n = counts.get(sector)
        n_str = "" if n is None or (isinstance(n, float) and pd.isna(n)) else str(int(n))
        rows.append(["", sector, f"{weight*100:.1f}%", n_str])

    legend_tbl = Table(rows, colWidths=[5 * mm, 38 * mm, 14 * mm, 13 * mm], rowHeights=[6 * mm] + [5.2 * mm] * len(exp))
    style = [
        ("BACKGROUND", (0, 0), (-1, 0), DARK_TEAL),
    ...
    outer = Table([[pie_img, legend_tbl]], colWidths=[82 * mm, 70 * mm])
...
def external_cashflows_table(cashflows_for_portfolio: pd.DataFrame) -> Optional[Table]:
    """
    Every external Cashflows-sheet entry (deposits/withdrawals) for this
    portfolio — Date, Amount — newest first. Deposits shown in green,
    withdrawals in red, matching the brand's positive/negative convention
    used everywhere else in this report. This is the raw external-money
    ledger, separate from the daily equity+cash ledger used for TWR.
    """
    if cashflows_for_portfolio is None or cashflows_for_portfolio.empty:
        return None
    cf = cashflows_for_portfolio.copy()
    cf["date"] = pd.to_datetime(cf["date"], errors="coerce")
    cf = cf.dropna(subset=["date"]).sort_values("date", ascending=False)
    if cf.empty:
        return None

    rows = [["Date", "Amount (INR)"]]
    for _, r in cf.iterrows():
        amt = float(r["amount"])
        rows.append([r["date"].date().isoformat(), fmt_inr(amt)])

    t = Table(rows, colWidths=[60 * mm, 60 * mm], repeatRows=1)
    style = [
        ("BACKGROUND", (0, 0), (-1, 0), DARK_TEAL),
        ("TEXTCOLOR", (0, 0), (-1, 0), WHITE),
        ("FONTNAME", (0, 0), (-1, 0), TITLE_STYLE.fontName),
    ...
    for i, (_, r) in enumerate(cf.iterrows(), start=1):
        color = POSITIVE_GREEN if float(r["amount"]) >= 0 else NEGATIVE_RED
...
def generate_portfolio_pdf_bytes(
    *,
    ...
    story.append(KeepTogether([
        Paragraph("Trailing Period Returns", SECTION_STYLE),
        HRFlowable(width="100%", color=ACCENT_TEAL, thickness=0.5),
        Spacer(1, 4 * mm),
        tr_content,
    ]))

    # Deliberate page break: Executive Summary + Summary Statistics + Trailing
    # Period Returns are meant to read as one self-contained "at a glance"
    # page — everything else starts fresh rather than however much of
    # Performance happens to fit in whatever room is left.
    ...
    tbl = top_holdings_table(
        holdings, prices, portfolio_value=portfolio_value, market_caps=market_caps, cash=cash_balance
    )
    ...
    # Sector Exposure and Market Cap Allocation are grouped into ONE
    # KeepTogether block (not two separate ones in sequence) so they're
    # guaranteed to land on the same page together — relying on "usually
    # fits" natural flow isn't reliable once content above them shifts size.
    story.append(Spacer(1, 10 * mm))
    sec = sector_pie_image(holdings, prices, sector_map=sector_map, cash=cash_balance, portfolio_value=portfolio_value)
    ...
    story.append(Spacer(1, 8 * mm))
    cf_tbl = external_cashflows_table(cashflows) if cashflows is not None else None
    cf_content = cf_tbl if cf_tbl is not None else Paragraph("No external cashflows recorded for this portfolio.", BODY_STYLE)
    story.append(KeepTogether([
        Paragraph("External Cashflows", SECTION_STYLE),
        HRFlowable(width="100%", color=ACCENT_TEAL, thickness=0.5),
        Spacer(1, 4 * mm),
        cf_content,
    ]))
...
def generate_portfolio_pptx_bytes(
    *,
    ...
    def _style_table(tbl, n_rows, n_cols, font_size=11):
    ...
    def _add_table(slide, rows, top, col_widths=None, left=Inches(0.5), width=Inches(12.3), row_height=0.4, font_size=11):
        ...
        _style_table(tbl, n_rows, n_cols, font_size=font_size)
        return gframe
    ...
    # ---- Slide 1: Title ----
    slide = _add_slide()
    tb = slide.shapes.add_textbox(Inches(1), Inches(1.6), Inches(11.3), Inches(1.2))
    tf = tb.text_frame
    ...
    tb2 = slide.shapes.add_textbox(Inches(1), Inches(2.7), Inches(11.3), Inches(0.6))
    tf2 = tb2.text_frame
    ...
    tb3 = slide.shapes.add_textbox(Inches(1), Inches(3.3), Inches(11.3), Inches(0.5))
    tf3 = tb3.text_frame
    ...
    # ---- Slide 2: Executive Summary + Summary Statistics + Trailing Period Returns ----
    # All three on one slide, matching the PDF's "page 1" grouping — compact
    # row heights/fonts specifically here since a fixed-size slide has much
    # less room than a flowing PDF page.
    slide = _add_slide()
    _add_title(slide, "Executive Summary")
    _add_body_text(
        slide,
        summary_text(portfolio_name=str(portfolio_name), stats_row=stats_row, returns=returns),
        top=Inches(0.95), height=Inches(0.9), size=12,
    )
    stats_bottom = Inches(1.9)
    if stats_row is not None:
        rows = [["Metric", "Value"]]
        if portfolio_value is not None:
            rows.append(["Current Portfolio Value", fmt_inr(portfolio_value)])
        ...
        rows.append(["CVaR 99% (Daily)", _safe_fmt_pct(stats_row.get("CVaR 99% (Daily)"))])
        rows.append(["Max Drawdown", _safe_fmt_pct(stats_row.get("Max Drawdown"))])
        row_h = 0.3 if len(rows) <= 12 else 0.24
        _add_table(
            slide, rows, top=Inches(1.9), col_widths=[Inches(5.2), Inches(2.3)], width=Inches(7.5),
            row_height=row_h, font_size=9,
        )
        stats_bottom = Inches(1.9 + row_h * len(rows) + 0.3)
    ...
    if trailing_returns is not None and not trailing_returns.empty:
        ...
        for label, row in trailing_returns.iterrows():
            tr_rows.append([label] + ["" if pd.isna(v) else f"{v*100:.2f}%" for v in row])
        # Placed beside the stats table (not stacked below it) — the stats
        # table can run to 10+ rows depending on how many benchmarks are
        # configured, and a 7.5" tall slide doesn't reliably have room for
        # both stacked vertically; side-by-side uses the slide's width
        # instead, which this table (short and wide) is well suited to.
        _add_table(
            slide, tr_rows, top=Inches(1.9), left=Inches(8.3), width=Inches(4.5),
            row_height=0.32, font_size=9,
        )
        tr_caption = slide.shapes.add_textbox(Inches(8.3), Inches(1.9 + 0.32 * len(tr_rows) + 0.1), Inches(4.5), Inches(1.0))
        tr_caption.text_frame.word_wrap = True
        tr_caption.text_frame.text = "Trailing Period Returns — Portfolio TWR vs. Nifty 500, with Alpha per period."
        tr_caption.text_frame.paragraphs[0].runs[0].font.size = Pt(9)
        tr_caption.text_frame.paragraphs[0].runs[0].font.color.rgb = MID_GRAY_RGB
        tr_caption.text_frame.paragraphs[0].runs[0].font.italic = True
    ...
    if tickers_h:
        ...
        mv_all = (latest_h * latest_p)
        mv = mv_all[mv_all > 0].sort_values(ascending=False)
        # Weight = % of TRUE portfolio value (equity + cash), not just the
        # sum of equity positions shown here — matches top_holdings_table's
        # PDF logic exactly, same reasoning: excluding cash from the
        # denominator silently inflates every displayed weight.
        if portfolio_value is not None and pd.notna(portfolio_value) and abs(portfolio_value) > 1e-9:
            denom = float(portfolio_value)
        else:
            denom = mv_all.sum()
        ...
        rows = [["Ticker", "Market Cap", "Weight"]]
        for ticker, val in mv.items():
            weight_str = f"{val/denom*100:.1f}%" if denom and abs(denom) > 1e-9 else "N/A"
            cat = categories.get(ticker) if not categories.empty else None
            mcap_str = "" if cat is None or cat in ("ETF", "REIT") else cat
            rows.append([ticker.replace(".NS", "").replace(".BO", ""), mcap_str, weight_str])
        if cash_balance is not None and pd.notna(cash_balance):
            cash_weight = f"{cash_balance/denom*100:.1f}%" if denom and abs(denom) > 1e-9 else "N/A"
            rows.append(["Cash", "", cash_weight])
        row_h = 0.32 if len(rows) <= 18 else max(0.18, 5.9 / len(rows))
        _add_table(slide, rows, top=Inches(1.1), row_height=row_h, font_size=10 if row_h >= 0.28 else 8)

    # ---- Slide 5: Sector Exposure + Market Cap Allocation (same slide) ----
    exp, counts = sector_exposure_weights_and_counts(
        holdings, prices, sector_map or {}, cash=cash_balance, portfolio_value=portfolio_value
    )
    cap_split_clean = normalize_cap_split_for_report(cap_split)
    have_sector = not exp.empty
    have_cap = cap_split_clean is not None and not cap_split_clean.empty
    if have_sector or have_cap:
        slide = _add_slide()
        if have_sector:
            _add_title(slide, "Sector Exposure", top=Inches(0.3))
            fig, colors_hex_pptx = sector_pie_figure_clean(exp)
            _add_image_fig(slide, fig, top=Inches(1.0), left=Inches(0.3), width=Inches(3.4))

            n_rows = len(exp) + 1
            tbl_top, tbl_left = Inches(1.0), Inches(3.9)
            tbl_width = Inches(3.6)
            gframe = slide.shapes.add_table(n_rows, 4, tbl_left, tbl_top, tbl_width, Inches(0.3 * n_rows))
            tbl = gframe.table
            for r in range(n_rows):
                tbl.rows[r].height = Inches(0.3)
            tbl.columns[0].width = Inches(0.25)
            ...
            for r, (sector, weight) in enumerate(exp.items(), start=1):
                ...
                n = counts.get(sector)
                n_str = "" if n is None or (isinstance(n, float) and pd.isna(n)) else str(int(n))
                vals = [sector, f"{weight*100:.1f}%", n_str]
        ...
        if have_cap:
            # Kept well clear of the logo, which sits at x=11.6"-~12.3" —
            # everything here ends by x=11.3" with margin to spare.
            title_left = Inches(0.5) if not have_sector else Inches(7.8)
            tb_cap = slide.shapes.add_textbox(title_left, Inches(0.3), Inches(3.4), Inches(0.5))
            tf_cap = tb_cap.text_frame
            tf_cap.text = "Market Cap Allocation"
            run_cap = tf_cap.paragraphs[0].runs[0]
            run_cap.font.size = Pt(18) if have_sector else Pt(24)
            run_cap.font.bold = True
            run_cap.font.color.rgb = BRAND_INDIGO
            ...
            for txt, color in zip(autotexts, colors_ordered):
                txt.set_color(_contrast_text_color(color))
            fig.tight_layout()
            cap_left = Inches(7.8) if have_sector else Inches(3.4)
            _add_image_fig(slide, fig, top=Inches(1.0), left=cap_left, width=Inches(3.3))
    ...
    if cashflows is not None and not cashflows.empty:
        ...
        if not cf.empty:
            slide = _add_slide()
            _add_title(slide, "External Cashflows")
            rows = [["Date", "Amount (INR)"]]
            for _, r in cf.iterrows():
                rows.append([r["date"].date().isoformat(), fmt_inr(float(r["amount"]))])
            row_h = 0.32 if len(rows) <= 18 else max(0.18, 5.9 / len(rows))
            gframe = _add_table(slide, rows, top=Inches(1.1), left=Inches(0.5), width=Inches(6), row_height=row_h, font_size=10)
            tbl = gframe.table
            for i, (_, r) in enumerate(cf.iterrows(), start=1):
                amt_color = BRAND_GREEN if float(r["amount"]) >= 0 else BRAND_RED
                for p in tbl.cell(i, 1).text_frame.paragraphs:
                    p.font.color.rgb = amt_color
...
APP_BUILD_TAG = "build-2026-08-19-marketcap-classification-not-figure-20"
...
with overview_tab:
    ...
    # Numeric dtype throughout (not pre-formatted strings) so the interactive
    # table actually sorts by VALUE — "+100.00%" sorting before "+15.33%" as
    # text (because '0' < '5' at the second character) is exactly the
    # failure mode this avoids. st.column_config formats the display without
    # touching the underlying number, so sort stays correct.
    # Values well outside what's plausible for a diversified equity
    # portfolio can't carry a "⚠" text marker anymore (that would make the
    # cell a string again) — flagged as a general caption below instead.
    stats_num = pd.DataFrame(index=stats_disp.index.astype(str))
    col_config: dict = {}
    flag_state = {"any": False}

    def _add_pct_col(name: str, series: pd.Series, lo: float = None, hi: float = None):
        stats_num[name] = series.to_numpy(dtype=float) * 100
        col_config[name] = st.column_config.NumberColumn(format="%.2f%%")
        if lo is not None or hi is not None:
            bad = series.apply(lambda v: pd.notna(v) and ((lo is not None and v < lo) or (hi is not None and v > hi)))
            if bool(bad.any()):
                flag_state["any"] = True

    def _add_float_col(name: str, series: pd.Series, lo: float = None, hi: float = None):
        stats_num[name] = series.to_numpy(dtype=float)
        col_config[name] = st.column_config.NumberColumn(format="%.2f")
        if lo is not None or hi is not None:
            bad = series.apply(lambda v: pd.notna(v) and ((lo is not None and v < lo) or (hi is not None and v > hi)))
            if bool(bad.any()):
    ...
    if "Period Return (TWR)" in stats_disp.columns:
        _add_pct_col("Period Return (TWR)", stats_disp["Period Return (TWR)"], lo=-0.95, hi=5.0)
    if "Period Volatility (TWR)" in stats_disp.columns:
        _add_pct_col("Period Volatility (TWR)", stats_disp["Period Volatility (TWR)"], lo=0, hi=1.5)
    if "Sharpe" in stats_disp.columns:
        _add_float_col("Sharpe", stats_disp["Sharpe"], lo=-10, hi=10)
    if "Sortino" in stats_disp.columns:
        _add_float_col("Sortino", stats_disp["Sortino"], lo=-10, hi=10)
    ...
    for col in stats_disp.columns:
        if col.startswith("Beta (") or col.startswith("Treynor ("):
            _add_float_col(col, stats_disp[col], lo=-5, hi=5)
        elif col.startswith("Jensen Alpha ("):
            _add_pct_col(f"{col} (TWR trailing period)", stats_disp[col], lo=-2.0, hi=2.0)
    if "CVaR 99% (Daily)" in stats_disp.columns:
        _add_pct_col("CVaR 99% (Daily)", stats_disp["CVaR 99% (Daily)"])
    if "Max Drawdown" in stats_disp.columns:
        _add_pct_col("Max Drawdown", stats_disp["Max Drawdown"])

    st.dataframe(stats_num, use_container_width=True, column_config=col_config)
    if flag_state["any"]:
        st.caption(
    ...
    if pnl_df.empty:
        ...
        # Also show as table — kept numeric (raw ₹ amount) with column_config
        # formatting, not pre-formatted strings, so the interactive sort
        # actually orders by value instead of by text.
        pnl_display = pnl_df.copy()
        pnl_display.columns = ["Realized P&L", "Unrealized P&L", "Total P&L"]
        st.dataframe(
            pnl_display,
            use_container_width=True,
            column_config={c: st.column_config.NumberColumn(format="₹%.0f") for c in pnl_display.columns},
        )
    ...
    if cashflows_df.empty:
        ...
        st.dataframe(
            summary_disp,
            use_container_width=True,
            column_config={"Net cash contributed (₹)": st.column_config.NumberColumn(format="₹%.0f")},
        )
        st.caption(
...
with client_tab:
    ...
    if not client_ledger.empty:
        ...
        perf_table_num = perf_table * 100
        st.subheader("Performance Table")
        st.dataframe(
            perf_table_num,
            use_container_width=True,
            column_config={c: st.column_config.NumberColumn(format="%.2f%%") for c in perf_table_num.columns},
        )
        ...
        if trailing_table.empty:
            ...
            trailing_num = trailing_table * 100
            st.dataframe(
                trailing_num,
                use_container_width=True,
                column_config={c: st.column_config.NumberColumn(format="%.2f%%") for c in trailing_num.columns},
            )
    else:
    ...
    if benchmarks:
        ...
        for name in benchmarks.keys():
            alpha_val = client_metrics.get(f"Jensen Alpha ({name})")
            alpha_pct = float(alpha_val) * 100 if pd.notna(alpha_val) else np.nan
            bench_rows.append(
                {
                    "Benchmark": name,
                    "Beta": client_metrics.get(f"Beta ({name})"),
                    "Jensen Alpha (TWR trailing period)": alpha_pct,
                    "Treynor": client_metrics.get(f"Treynor ({name})"),
                }
            )
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Jensen Alpha (TWR trailing period)": st.column_config.NumberColumn(format="%.2f%%"),
                "Treynor": st.column_config.NumberColumn(format="%.2f"),
            },
        )
    ...
    port_value_for_weight = float(client_ledger["portfolio_value"].iloc[-1]) if not client_ledger.empty else None
    cash_for_weight = float(client_ledger["cash"].iloc[-1]) if not client_ledger.empty else None
    mkt_caps = pd.Series(dtype=float)
    if holdings.empty:
        ...
        # All holdings (no top-N cutoff), Ticker / Market Cap / Weight — Weight
        # is % of TRUE total portfolio value (equity + cash), matching the
        # PDF/PPTX Holdings table exactly, not just % of the equity shown here.
        if held_tickers:
            with st.spinner("Fetching market caps…"):
                mkt_caps = _fetch_market_caps_cached(held_tickers)
            latest_p = prices.loc[prices.index[-1], list(held_tickers)]
            mv = latest[list(held_tickers)] * latest_p
            denom = port_value_for_weight if port_value_for_weight and abs(port_value_for_weight) > 1e-9 else mv.sum()
            categories = categorize_market_caps_inr(mkt_caps)

            holdings_rows = []
            for ticker in mv.sort_values(ascending=False).index:
                cat = categories.get(ticker)
                mcap_label = "" if cat is None or cat in ("ETF", "REIT") else cat
                weight_val = mv[ticker] / denom * 100 if denom and abs(denom) > 1e-9 else np.nan
                holdings_rows.append({"Ticker": ticker.replace(".NS", "").replace(".BO", ""), "Market Cap": mcap_label, "Weight": weight_val})
            holdings_disp = pd.DataFrame(holdings_rows).set_index("Ticker")
            st.dataframe(
                holdings_disp,
                use_container_width=True,
                column_config={
            ...
            # Cash is shown as a separate element, not a row in the sortable
            # table above — Streamlit's interactive dataframe sort would move
            # a "Cash" row wherever its value ranks the moment someone sorts
            # by a column, and there's no way to pin a row to the bottom of
            # a sortable st.dataframe. A physically separate element is the
            # only way to guarantee it always reads as the last line.
            if cash_for_weight is not None:
                cash_weight = cash_for_weight / denom * 100 if denom and abs(denom) > 1e-9 else np.nan
                cc1, cc2 = st.columns(2)
                cc1.metric("Cash", _fmt_inr(cash_for_weight))
                cc2.metric("Cash — Weight", _fmt_pct(cash_weight / 100 if pd.notna(cash_weight) else np.nan))
        else:
            st.info("No currently-held tickers with price data.")

    st.subheader("Market cap allocation")
    cap_split = pd.Series(dtype=float)
    if not holdings.empty and not mkt_caps.empty:
    ...
    if not holdings.empty and held_tickers:
        sector_split, sector_counts = sector_exposure_weights_and_counts(
            holdings, prices, effective_sector_map, cash=cash_for_weight, portfolio_value=port_value_for_weight
        )
        if sector_split.empty:
            ...
            plt.close(fig_sec)
            with legend_col:
                def _count_str(sector):
                    n = sector_counts.get(sector)
                    if n is None or (isinstance(n, float) and pd.isna(n)):
                        return ""
                    return str(int(n))
                ...
                legend_rows = "".join(
                    ...
                    f'<td style="padding:3px 8px;text-align:right;">{_count_str(sector)}</td></tr>'
    ...
    if not holdings.empty:
        ...
        if attrib is not None and not attrib.empty:
            ...
            contrib_df["Contribution to Return"] = contrib_df["contribution"] * 100
            ...
            if total_return_sum and abs(total_return_sum) > 1e-9:
                contrib_df["% of Total Return"] = (contrib_df["contribution"] / total_return_sum) * 100
            else:
                contrib_df["% of Total Return"] = np.nan
            ...
            st.dataframe(
                ...
                column_config={
                    "Contribution to Return": st.column_config.NumberColumn(format="%.2f%%"),
                    "% of Total Return": st.column_config.NumberColumn(format="%.2f%%"),
                },
            )
        else:
        ...
        if rc is not None and not rc.empty:
            ...
            if pd.notna(port_vol):
                rc_df["Contribution to Volatility"] = (rc_df["share"] * port_vol) * 100
            else:
                rc_df["Contribution to Volatility"] = np.nan
            rc_df["% of Total Risk"] = rc_df["share"] * 100
            rc_df = rc_df[["Contribution to Volatility", "% of Total Risk"]]
            st.dataframe(
                rc_df,
                use_container_width=True,
                height=min(35 * (len(rc_df) + 1), 480),
                column_config={
                    "Contribution to Volatility": st.column_config.NumberColumn(format="%.2f%%"),
                    "% of Total Risk": st.column_config.NumberColumn(format="%.2f%%"),
                },
            )
        else:
        ...
        if rr_df.empty:
            ...
            rr_disp["% of Total Return"] = rr_disp["% of Total Return"] * 100
            rr_disp["% of Total Risk"] = rr_disp["% of Total Risk"] * 100
            st.dataframe(
                rr_disp,
                use_container_width=True,
                column_config={
        ...
        if holdings.empty or pr.empty:
            ...
            try:
                pdf_bytes = generate_portfolio_pdf_bytes(
                    ...
                    market_caps=mkt_caps if "mkt_caps" in locals() and isinstance(mkt_caps, pd.Series) else None,
                    cashflows=client_cash if isinstance(client_cash, pd.DataFrame) and not client_cash.empty else None,
                )
                st.download_button(
                    "Download PDF",
                    data=pdf_bytes,
