...
with st.sidebar:
    ...
    st.divider()
    st.header("Market-cap buckets")
    st.caption("Large: > ₹1,00,000 cr • Mid: ₹30,000–₹1,00,000 cr • Small: < ₹30,000 cr")
...
shares_out = _fetch_shares_outstanding_cached(tickers) if tickers else pd.Series(dtype="float64")
market_caps = pd.Series(dtype="float64")
if not shares_out.empty:
    # Market cap (INR) = latest price (from downloaded prices) * shares outstanding
    latest_prices = prices.reindex(columns=shares_out.index).ffill().iloc[-1]
    market_caps = (latest_prices * shares_out).dropna()
...
with overview_tab:
    ...
    stats_fmt = pd.DataFrame(index=stats_disp.index)
    if "Annual Return" in stats_disp.columns:
        stats_fmt["Annual Return"] = _fmt_col_pct(stats_disp["Annual Return"])
    if "Volatility" in stats_disp.columns:
        stats_fmt["Volatility"] = _fmt_col_pct(stats_disp["Volatility"])
    if "Sharpe Ratio" in stats_disp.columns:
        stats_fmt["Sharpe Ratio"] = _fmt_col_float(stats_disp["Sharpe Ratio"])
    ...
    if market_caps.empty:
        st.info("Shares-outstanding unavailable for market-cap bucketing (cap split may be missing).")
...
with client_tab:
    ...
    if not holdings.empty:
        ...
        if not cap_split.empty:
            _bar_chart_from_series(cap_split, title="Cap split", value_label="Weight", top_n=10)
        else:
