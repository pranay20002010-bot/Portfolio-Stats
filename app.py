...
def top_holdings_table(
    holdings: pd.DataFrame, prices: pd.DataFrame, n: int = 12, portfolio_value: Optional[float] = None
) -> Optional[Table]:
    """
    Weight = each holding's market value as a % of the TRUE total
    portfolio value (equity + cash), not just the sum of equity positions
    shown in this table. Dividing by the sum of the displayed holdings
    alone is wrong two ways at once: it excludes cash entirely (so a
    ₹50k position in a ₹10L portfolio would show as some inflated
    equity-only share instead of 5%), and if there are more than `n`
    holdings, it silently excludes the ones cut off by the top-N limit
    too, inflating every shown weight further.

    Falls back to "% of shown equity" (the old, wrong-denominator
    behavior) only if the caller genuinely has no portfolio value to pass
    in — better to show *something* than nothing, but every real call
    site should be passing the actual portfolio value.
    """
    tickers = [t for t in holdings.columns if t in prices.columns]
    if not tickers:
        return None
...
def generate_portfolio_pdf_bytes(
    *,
    ...
    tbl = top_holdings_table(holdings, prices, n=15, portfolio_value=portfolio_value)
...
def generate_portfolio_pptx_bytes(
    *,
    ...
    if tickers_h:
        ...
        # PDF logic exactly, same reasoning: excluding cash and/or
        # off-screen holdings from the denominator silently inflates every
        # displayed weight.
        if portfolio_value is not None and pd.notna(portfolio_value) and abs(portfolio_value) > 1e-9:
            denom = float(portfolio_value)
        else:
...
APP_BUILD_TAG = "build-2026-08-19-sortable-numeric-tables-18"
...
with client_tab:
    ...
    if holdings.empty:
        ...
        holdings_col_config = {"Qty": st.column_config.NumberColumn(format="%.0f")}
        if "avg_buy_price" in port_pos.columns and not port_pos.empty:
            ...
            holdings_col_config["Avg Buy Price (₹)"] = st.column_config.NumberColumn(format="₹%.2f")
        st.dataframe(latest, use_container_width=True, column_config=holdings_col_config)
