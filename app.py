...
def _heatmap(df: pd.DataFrame, *, title: str):
    ...
    # Custom colormap: green(-1) → yellow(0) → red(+1)
    ...
    for i in range(len(df.index)):
        for j in range(len(df.columns)):
            v = df.iat[i, j]
            # Use white text on dark ends, black in the middle
...
with client_tab:
    ...
    m5.metric("99% VaR (Daily)", _fmt_pct(client_metrics.get("VaR 99% (Daily)")))
    ...
    if not pr.empty and len(pr) >= 10:
        import matplotlib.pyplot as plt
        import matplotlib.ticker as mticker
        var99_daily = float(pr.quantile(0.01))
        ...
        ax_hist.axvline(var99_daily, color="#C0392B", linewidth=1.5, linestyle="--",
                        label=f"99% VaR: {var99_daily*100:.2f}%")
