...
BENCHMARK_NIFTY500_TICKER = "^CRSLDX"
BENCHMARK_NIFTY50_TICKER = "^NSEI"
...
def download_prices(
    ...
    if include_benchmark:
        ...
        for bt in benches:
            try:
                bench = yf_module.download(bt, start=start, auto_adjust=True, progress=False)
                if bench is not None and not bench.empty and "Close" in bench.columns:
                    prices[bt] = bench["Close"]
            except Exception:
                pass
