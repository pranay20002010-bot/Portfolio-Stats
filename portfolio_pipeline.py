...
def annualized_return_from_twr(twr_returns: pd.Series) -> float:
    ...
    Uses geometric compounding: (prod(1+r_i))^(252/n) - 1
    """
...
def sortino_ratio(returns: pd.Series, rf_annual: float = 0.0) -> float:
    ...
    if cumulative <= 0:
        return np.nan
    ann_ret = float(cumulative ** (252.0 / n) - 1)
...
def jensen_alpha(
    ...
    if pd.isna(beta):
        return np.nan
    cum_p = float((1 + r).prod())
    cum_b = float((1 + b).prod())
    n_p, n_b = len(r), len(b)
    if cum_p <= 0 or cum_b <= 0 or n_p < 2 or n_b < 2:
        return np.nan
    ann_p = float(cum_p ** (252.0 / n_p) - 1)
    ann_b = float(cum_b ** (252.0 / n_b) - 1)
...
def treynor_ratio(
    ...
    if cumulative <= 0:
        return np.nan
    ann_p = float(cumulative ** (252.0 / n) - 1)
