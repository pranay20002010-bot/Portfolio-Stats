...
def portfolio_returns(nav_df: pd.DataFrame) -> pd.DataFrame:
    return nav_df.pct_change().dropna(how="all")
...
def download_prices(
    ...
    for ticker in tickers_ns:
        bse_ticker = bse_ticker_from_ns(ticker)
        try:
            temp = yf_module.download(bse_ticker, start=start, auto_adjust=True, progress=False)
            if temp is None or getattr(temp, "empty", True):
            ...
            if "Close" in temp.columns:
                close = temp["Close"]
            elif isinstance(temp.columns, pd.MultiIndex) and ("Close", bse_ticker) in temp.columns:
                close = temp[("Close", bse_ticker)]
            ...
            successful.append(f"{ticker} -> {bse_ticker}")
        except Exception:
            failed.append(ticker)
...
def categorize_market_caps_inr(
    ...
    *,
    large_crore: float = 20000,
    mid_crore: float = 5000,
    small_crore: float = 1000,
) -> pd.Series:
    """
    Categorize tickers by market cap (INR).
    Defaults (crore INR): Large >= 20,000; Mid >= 5,000; Small >= 1,000; else Micro.
    """
    caps = market_caps_inr.copy()
    caps = pd.to_numeric(caps, errors="coerce")
    large = large_crore * 1e7
    mid = mid_crore * 1e7
    small = small_crore * 1e7
