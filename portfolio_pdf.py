...
def normalize_cap_split_for_report(cap_split: Optional[pd.Series]) -> Optional[pd.Series]:
    ...
    - Merge Micro Cap into Small Cap
    ...
    if s.empty:
        return None
    s = s.copy()
    if "Micro Cap" in s.index:
        s["Small Cap"] = float(s.get("Small Cap", 0.0)) + float(s["Micro Cap"])
        s = s.drop(index=["Micro Cap"])
...
def summary_text(
    *,
    ...
    ann = stats_row.get("Annual Return") if stats_row is not None else np.nan
    vol = stats_row.get("Volatility") if stats_row is not None else np.nan
    sharpe = stats_row.get("Sharpe Ratio") if stats_row is not None else np.nan
    ...
    parts = [
        ...
        f"Annualised return: {_pct(ann)}; annualised volatility: {_pct(vol)}; Sharpe ratio: {_flt(sharpe)}; max drawdown: {_pct(mdd)}.",
    ]
    if w_txt:
...
def generate_portfolio_pdf_bytes(
    *,
    ...
    story = []
    # Cover + logo
    logo_file = Path(__file__).resolve().parent / "assets" / "vika_logo.png"
    if logo_file.exists():
        try:
            story.append(Image(str(logo_file), width=45 * mm, height=18 * mm))
        except Exception:
            pass
    ...
    if stats_row is not None:
        rows = [
            ["Annual Return", fmt_pct(float(stats_row.get("Annual Return")))],
            ["Volatility", fmt_pct(float(stats_row.get("Volatility")))],
            [
                "Sharpe Ratio",
                f"{stats_row.get('Sharpe Ratio'):.2f}" if pd.notna(stats_row.get("Sharpe Ratio")) else "N/A",
            ],
    ...
    story.append(Spacer(1, 6 * mm))
    story.append(Paragraph("Large / Mid / Small Cap Split", SECTION_STYLE))
    story.append(HRFlowable(width="100%", color=ACCENT_TEAL, thickness=0.5))
    story.append(Spacer(1, 4 * mm))
    cap_norm = normalize_cap_split_for_report(cap_split)
    cap_img = pie_series_image(cap_norm, "Cap split") if cap_norm is not None else None
    if cap_img is not None:
        story.append(cap_img)
    else:
        story.append(Paragraph("Cap split not available.", BODY_STYLE))
